...
public class AtividadeDAO implements GenericDAO{
    ...
    public Boolean inserir(Object objeto) {
        ...
        String sql = "insert into atividade(descricao, situacao, status, dataatividade,dataprazo,documento, iddisciplina, idturma, pontuacaomax, pontuacaofinal) values (?,?,?,?,?,?,?,?,?,?);";
        try{
            ...
            stmt.setInt(8, oAtividade.getTurma().getIdTurma());
            stmt.setInt(9, oAtividade.getPontuacaomax());
            stmt.setInt(10, oAtividade.getPontuacaofinal());
    ...
    public Boolean alterar(Object objeto) {
        Atividade oAtividade = (Atividade) objeto;
        PreparedStatement stmt = null;
        String sql = "update atividade set descricao=?, status=?, dataatividade=?, dataprazo=?, documento=?, iddisciplina=?,idturma=?, pontuacaomax=?, pontuacaofinal=? where idatividade=?";
        try{
            stmt = conexao.prepareStatement(sql);
            stmt.setString(1, oAtividade.getDescricao());
            ...
            stmt.setInt(7, oAtividade.getTurma().getIdTurma());
            stmt.setInt(8, oAtividade.getPontuacaomax());
            stmt.setInt(9, oAtividade.getPontuacaofinal());
            stmt.setInt(10, oAtividade.getIdAtividade());
            stmt.execute();
            conexao.commit();
            return true;
        }catch (Exception ex){
   ...
   public Object carregar(int numero) {
        ...
        try{
            ...
            while(rs.next()){
                ...
                oAtividade.setPontuacaofinal(rs.getInt("pontuacaofinal"));
                ...
                oAtividade.setTurma((Turma) oTurmaDAO.carregar(rs.getInt("idturma")));
            }
            return oAtividade;
        }catch (Exception ex){
    ...
    public List<Object> listar(){   
        ...
        try{
            ...
            while (rs.next()){
                ...
                oAtividade.setDisciplina((Disciplina) oDisciplinaDAO.carregar(rs.getInt("iddisciplina")));
                TurmaDAO oTurmaDAO = null;
                try{
                    oTurmaDAO = new TurmaDAO();
                }catch(Exception ex){
                    System.out.println("Erro buscar turma"+ex.getMessage());
                    ex.printStackTrace();
                }
                oAtividade.setTurma((Turma) oTurmaDAO.carregar(rs.getInt("idturma")));
    ...
    public Boolean entregar(int numero){
        int idAtividade = numero;
        PreparedStatement stmt = null;
        String sql = "update atividade set status=? where idatividade=?";
         try{
            Atividade oAtividade = (Atividade) this.carregar(idAtividade);
            stmt = conexao.prepareStatement(sql);
            if(oAtividade.getStatus().equals("E"))
                stmt.setString(1, "N"); 
            else stmt.setString(1, "E");
            stmt.setInt(2, idAtividade);
            stmt.execute();
            conexao.commit();
            return true;
        }catch (Exception ex){
            System.out.println("Problemas ao entregar a Atividade! Erro: "+ex.getMessage());
            try{
                conexao.rollback();
            }catch(SQLException e){
                System.out.println("Erro rollback "+e.getMessage());
    ...
    /*public Boolean entregar(int idAtividade){
    PreparedStatement stmt = null;
    String sql = "update atividade set status=? where idatividade=?";
        ...
        try{
            ...
            while(rs.next()){
                ...
                strJson += "{\"idAtividade\":"+rs.getInt("idatividade")+","
                        ...
                        +"\"pontuacaofinal\":"+rs.getInt("pontuacaofinal")+"\"}";
                i++;
            }
            strJson += "]";
        }catch(Exception e){
